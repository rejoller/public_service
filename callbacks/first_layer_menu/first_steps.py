...
from utils.text_messages  import FIRST_STEP
from kb.second_step import feedback_markup


router = Router()

@router.callback_query(F.data == 'first_step')
async def handle_waiting_for_choise(query: CallbackQuery, session: AsyncSession, bot: Bot):
    ...
    await query.message.answer_photo(parse_mode='HTML', reply_markup=feedback_markup, photo=FIRST_STEP, show_caption_above_media= True)
